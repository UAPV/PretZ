<?php



/**
 * Skeleton subclass for performing query and update operations on the 'category' table.
 *
 * 
 *
 * This class was autogenerated by Propel 1.5.3 on:
 *
 * Tue Sep 28 15:53:16 2010
 *
 * You should add additional methods to this class to meet the
 * application requirements.  This class will only be generated as
 * long as it does not already exist in the output directory.
 *
 * @package    propel.generator.lib.model
 */
class CategoryQuery extends BaseCategoryQuery {

    /*
     * @return array of Category
     *
     *  Allow to get all the category of the database
     */
    public static function getAllCateg()
    {
        $categ = self::create()
        ->find();

        return $categ;
    }

    /*
     * @param id of the product
     * @return a Category object
     *
     * Allow to find category of an product thanks its id
     */
    public static function getCategForOneProduct($idProd)
    {
        $categ = self::create()
                    ->useProductCategQuery()
                        ->filterByIdProduct($idProd)
                    ->endUse()
                ->findOne();

        return $categ;
    }


 
        
} // CategoryQuery
